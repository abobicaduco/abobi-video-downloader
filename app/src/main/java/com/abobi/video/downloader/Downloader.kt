...
import com.abobi.video.downloader.App.Companion.applicationScope
import com.abobi.video.downloader.App.Companion.context
import com.abobi.video.downloader.App.Companion.startService
import com.abobi.video.downloader.App.Companion.stopService
import com.abobi.video.downloader.database.objects.CommandTemplate
import com.abobi.video.downloader.util.COMMAND_DIRECTORY
...
import com.abobi.video.downloader.util.VideoClip
import com.abobi.video.downloader.util.VideoInfo
import com.abobi.video.downloader.util.toHttpsUrl
...
object Downloader {
    ...
    private val mutableCollectionInvestigation =
        MutableStateFlow<CollectionInvestigationResult?>(null)
    ...
    val collectionInvestigation = mutableCollectionInvestigation.asStateFlow()
    ...
    private fun areDownloadedFilesValid(paths: List<String>): Boolean =
        paths.any { path -> File(path).exists() && File(path).length() > 0L }
    ...
    fun updateCollectionInvestigation(result: CollectionInvestigationResult?) {
        mutableCollectionInvestigation.update { result }
        result?.let { updatePlaylistResult(it.playlist) }
    }
    ...
    fun quickDownload(
        ...
        applicationScope.launch(Dispatchers.IO) {
            if (!StoragePermissionHelper.hasStorageAccess(context)) {
                ToastUtil.makeToastSuspend(context.getString(R.string.storage_permission_rationale))
                return@launch
            }
    ...
    private suspend fun quickDownloadSingleVideo(
        url: String,
        videoInfo: VideoInfo,
        ...
        DownloadUtil.downloadVideo(
            ...
            if (!downloadPreferences.privateMode &&
                (paths.isEmpty() || !areDownloadedFilesValid(paths))
            ) {
                manageDownloadError(
                    th = Exception(context.getString(R.string.download_file_not_found)),
                    url = url,
                    title = videoInfo.title,
                    isFetchingInfo = false,
                    notificationId = notificationId,
            ...
            ToastUtil.makeToastSuspend(
                context.getString(R.string.download_success_toast).format(videoInfo.title)
            )
            val text = context.getString(
                if (paths.isEmpty()) R.string.status_completed else R.string.download_finish_notification
            )
            FileUtil.createIntentForOpeningFile(paths.firstOrNull()).run {
                NotificationUtil.finishNotification(
                    notificationId,
                    title = videoInfo.title,
                    text = text,
...
//        )
        ...
        return DownloadUtil.downloadVideo(
            ...
            val text =
                context.getString(if (paths.isEmpty()) R.string.status_completed else R.string.download_finish_notification)
            FileUtil.createIntentForOpeningFile(paths.firstOrNull()).run {
                NotificationUtil.finishNotification(
                    notificationId,
                    title = videoInfo.title,
