...
import com.abobi.video.downloader.ui.component.PreferenceSwitchWithContainer
import com.abobi.video.downloader.util.AUTO_SUBTITLE
import com.abobi.video.downloader.util.AUTO_TRANSLATED_SUBTITLES
import com.abobi.video.downloader.util.EMBED_SUBTITLE
import com.abobi.video.downloader.util.EXTRACT_AUDIO
import com.abobi.video.downloader.util.KEEP_SUBTITLE_FILES
...
fun SubtitlePreference(onNavigateBack: () -> Unit) {
    ...
    val hint by remember(embedSubtitle) {
        derivedStateOf {
            StringBuilder().apply {
                append(sponsorBlockText)
                if (embedSubtitle) {
                    append("\n\n")
                    append(embedSubtitleText)
                }
            }.toString()
        }
    }
